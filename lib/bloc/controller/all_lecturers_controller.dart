...
class AllLecturersController extends GetxController {

  List<Lecturer> lecturers=[];
  bool isLoading = false;
  GetStorage box = GetStorage();
  Future<void> fetchLecturers({int subjectId}) async {
    isLoading=true;
    update();
...
//   if (connectivityResult == ConnectivityResult.none)
   ...
   if (response.statusCode == 200 &&  extractedData['status']) {
     for (var item in extractedData['data']) {
       lecturers.add(Lecturer.fromJson(item));
     }
